...
const { ALL_MESSAGE_TYPE, CUSTOM_TYPE, CHAT_TYPE, CHANGE_MESSAGE_BODAY_TYPE } =
    messageType
/* login hxId */
const loginUserId = 'dbf959830e'//EaseChatClient''.user
...
const handleNickName = computed(() => {
    ...
    return (hxId) => {
        if (chatType === CHAT_TYPE.SINGLE) {
            return  hxId//friendList[hxId]?.nickname ||
        }
        if (chatType === CHAT_TYPE.GROUP) {
            const userInfoFromGroupNickname = ''
                // groupsInfos[id]?.groupMemberInfo?.[hxId]?.nickName
            const friendUserInfoNickname = hxId//friendList[hxId]?.nickname
...
<template>
    <div class="message_box" style="height:calc(100vh - 430px)">
        <div
            ...
            >
                ...
                <div class="message_box_card">
                    ...
                    <span
                        :class="[
                            isMyself(msgBody)
                                ? 'message_box_content_mine_right'
                                : ''
                        ]"
