...
import VueAt from '@/components/At/AtTextarea.vue' // for textarea
import iconEmoji from '@/assets/icon/icon-emoji.svg'
import iconTuku from '@/assets/icon/icon-tuku.svg'
import iconWenjian from '@/assets/icon/icon-wenjian.svg'
//EaseCallKit Invite
...
const atMembersList = computed(() => {
    ...
    const groupId = props.nowPickInfo.id
    console.log(props.nowPickInfo)
    //TODO text部分应为获取群组成员的自定义属性，待后续增加可设置自定在群组当中的自定义属性。
    if (groupId) {
        props.nowPickInfo.groupDetail.affiliations.map((obj,idx)=>{
            members.push({
                text:obj.member,
                value:obj.member
            })
...
<template>
    ...
    <!-- <template v-else-if="nowPickInfo.chatType === CHAT_TYPE.GROUP"> -->
        ...
        <at :members="atMembersList" name-key="text" @insert="onInsert" v-model:value="textContent">
            <template #header="{ item }">
                <div>
                    <div class="at_someone">
                        <el-avatar
                            shape="square"
                            :size="32"
                    ...
                    <p style="padding-top:12px;line-height: 24px;font-size: 15px;">群成员</p>
                </div>
            </template>
            <template #item="{ item }">
                <div class="at_someone">
            ...
            <div 
                class="editor chat_content_editable" 
                :contenteditable="true" 
                ref="editable"
                spellcheck="false"
                placeholder="请输入消息内容..."
    ...
    <!-- </template> -->
    <span style="position:absolute;bottom:28px;right:95px;color:#999;font-size:12px;">按Enter键发送</span>
    <el-button
