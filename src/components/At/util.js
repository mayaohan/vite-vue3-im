...
export function getPrecedingRange() {
  ...
  if (r) {
    ...
    const lastChild = range.endContainer.lastElementChild;
    // console.log(lastChild,range.endContainer)
    range.collapse(true)
...
export function getPrecedingRangeBackspace() {
  const r = getRange()
  if (r) {
    const range = r.cloneRange()
    range.collapse(true)
    ...
    const lastChild = range.endContainer.lastElementChild;
    // console.log(lastChild,range.endContainer)
    if (lastChild!=undefined && lastChild.nodeType === 1) {
      // console.log(111)
      // 如果最后一个子元素是标签（元素节点），则删除它
      range.endContainer.removeChild(lastChild);
    }else{
      if(lastChild!=null&&range.endContainer.length>0) {
        // console.log(222)
